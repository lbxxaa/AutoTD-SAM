...
from model.SAM import ImageEncoderViT, MaskDecoder, PromptEncoder, Sam, TwoWayTransformer
...
def load_from(sam, state_dict, image_size, vit_patch_size, encoder_global_attn_indexes):
    ...
    new_state_dict = {k: v for k, v in state_dict.items() if k in sam_dict.keys()}
    ...
    for k in sam_dict.keys():
        if k.startswith('mask_decoder1.') and k.replace('mask_decoder1.', 'mask_decoder.') in new_state_dict:
            sam_dict[k] = new_state_dict[k.replace('mask_decoder1.', 'mask_decoder.')]
